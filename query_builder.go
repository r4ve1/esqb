...
func (it *QueryGenerator) getQuery(value interface{}) elastic.Query {
	...
	if it.IsNested {
		return elastic.NewNestedQuery(it.NestPath, query).IgnoreUnmapped(true)
	} else {
		return query
	}
}
...
func (it *queryBuilder) Build() (elastic.Query, map[string]bool, error) {
	...
	err := errors.New("query build failed")
	if len(stack) != 1 {
		return nil, nil, err
	} else {
		query, ok := stack[len(stack)-1].Value.(elastic.Query)
		if !ok {
			return nil, nil, err
		} else {
			return query, it.queried, nil
		}
	}
}
